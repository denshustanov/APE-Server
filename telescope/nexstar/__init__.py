...
class NexstarHandController:
    ...
    def _to_bytes(arg):
        ...
        if isinstance(arg, NexstarCommand) or isinstance(arg, NexstarTrackingMode) or isinstance(arg,
                                                                                                 NexstarDeviceId) or isinstance(
                arg, NexstarPassthroughCommand):
            arg = arg.value
    ...
    def _read_binary(self, expected_response_length, check_and_remove_trailing_hash=True):
        ...
        if not (isinstance(expected_response_length, int) and expected_response_length > 0):
            raise NexstarUsageError(
                "_read_binary() failed: incorrect value for parameter 'expected_response_length': {}".format(
                    repr(expected_response_length)))

        if not isinstance(check_and_remove_trailing_hash, bool):
            raise NexstarUsageError(
                "_read_binary() failed: incorrect value for parameter 'check_and_remove_trailing_hash': {}".format(
                    repr(check_and_remove_trailing_hash)))

        response = self._device.read(expected_response_length)

        if len(response) != expected_response_length:
            raise NexstarProtocolError(
                "read_binary() failed: actual response length ({}) not equal to expected response length ({})".format(
    ...
    def getPosition(self, coordinateMode=NexstarCoordinateMode.AZM_ALT, highPrecisionFlag=True):
        ...
        if highPrecisionFlag:
            command = NexstarCommand.GET_POSITION_AZM_ALT_PRECISE if (
                        coordinateMode == NexstarCoordinateMode.AZM_ALT) else NexstarCommand.GET_POSITION_RA_DEC_PRECISE
            expected_response_length = 8 + 1 + 8 + 1
            denominator = 0x100000000
        else:
            command = NexstarCommand.GET_POSITION_AZM_ALT if (
                        coordinateMode == NexstarCoordinateMode.AZM_ALT) else NexstarCommand.GET_POSITION_RA_DEC
            expected_response_length = 4 + 1 + 4 + 1
            denominator = 0x10000

        self._write(command)

        response = self._read_ascii(expected_response_length=expected_response_length,
                                    check_and_remove_trailing_hash=True)
        response = response.split(",")
        if not (len(response) == 2):
            raise NexstarProtocolError("getPosition() failed: unexpected response ({})".format(response))

        response = (
        360.0 * int(response[0], 16) / float(denominator), 360.0 * int(response[1], 16) / float(denominator))
        return response

    def gotoPosition(self, firstCoordinate, secondCoordinate, coordinateMode=NexstarCoordinateMode.AZM_ALT,
                     highPrecisionFlag=True):

        # Initiate a "GoTo" command.

        if highPrecisionFlag:
            command = NexstarCommand.GOTO_POSITION_AZM_ALT_PRECISE if (
                        coordinateMode == NexstarCoordinateMode.AZM_ALT) else NexstarCommand.GOTO_POSITION_RA_DEC_PRECISE
            ...
            command = NexstarCommand.GOTO_POSITION_AZM_ALT if (
                        coordinateMode == NexstarCoordinateMode.AZM_ALT) else NexstarCommand.GOTO_POSITION_RA_DEC
    ...
    def setTrackingMode(self, tracking_mode):
        ...
        if not isinstance(tracking_mode, NexstarTrackingMode):
            raise NexstarUsageError(
                "_read_binary() failed: incorrect value for parameter 'tracking_mode': {}".format(repr(tracking_mode)))
    ...
    def setLocation(self, latitude, longitude):
        ...
        latitude_degrees = latitude_seconds // 3600;
        latitude_seconds %= 3600
        latitude_minutes = latitude_seconds // 60;
        latitude_seconds %= 60

        longitude_degrees = longitude_seconds // 3600;
        longitude_seconds %= 3600
        longitude_minutes = longitude_seconds // 60;
        longitude_seconds %= 60
    ...
    def passthrough(self, deviceId, command, expected_response_bytes):
        ...
        if not isinstance(deviceId, NexstarDeviceId):
            raise NexstarUsageError(
                "getDeviceVersion() failed: incorrect value for parameter 'deviceId': {}".format(repr(deviceId)))

        if not (isinstance(expected_response_bytes, int) and expected_response_bytes >= 0):
            raise NexstarUsageError(
                "passthrough() failed: incorrect value for parameter 'expected_response_bytes': {}".format(
                    repr(expected_response_bytes)))

        command_bytes = NexstarHandController._to_bytes(command)

        if not (1 <= len(command_bytes) <= 4):
            raise NexstarUsageError(
                "expected between 1 and 4 command bytes for passthrough command (received {})".format(len(command)))

        padding = bytes(4 - len(command_bytes))

        request = [NexstarCommand.PASSTHROUGH, len(command_bytes), deviceId, command_bytes, padding,
        ...
        # Receive response.
        try:
            response = self._read_binary(expected_response_length=expected_response_bytes + 1,
                                         check_and_remove_trailing_hash=True)
    ...
    def getDeviceVersion(self, deviceId):
        ...
        (versionMajor, versionMinor) = self.passthrough(deviceId, command=NexstarPassthroughCommand.GET_DEVICE_VERSION,
    ...
    def focuserGetPosition(self):
        position = self.passthrough(NexstarDeviceId.FOCUS_MOTOR, command=NexstarPassthroughCommand.FOCUSER_GET_POSITION,
    ...
    def focuserGotoInProgress(self):
        return not self.passthrough(NexstarDeviceId.FOCUS_MOTOR, [NexstarPassthroughCommand.FOCUSER_MOVE_DONE],
                                    1) == b'\xff'
...
def status_report(controller):
    ...
    print(
        "time .............................. : time = {}, dst = {}".format(time.strftime("%Y-%m-%d %H:%M:%S %Z"), dst))
